...
output=os.path.join(os.path.dirname(os.path.abspath(__file__)), "model", "skin_disease_model.pth")
...
print("Loading skin disease model. This may take a moment...")
...
def predict_disease_from_text(description):
    ...
    try:
        ...
        print(f"Gemini API Error (Text Prediction): {e}")
        return []
...
def generate_followup_questions(predictions):
    ...
    try:
        ...
        print(f"Gemini API Error (Follow-up): {e}")
        return ["Is your condition painful or itchy?", "How long have you had this issue?", "Have you tried any treatments so far?"]
...
camera = cv2.VideoCapture(0, cv2.CAP_DSHOW) # Using CAP_DSHOW for faster Windows initialization
...
def final_diagnosis_api():
    ...
    try:
        ...
        print(f"Gemini API Error (Final Disease): {e}")
        final_disease = "Unknown Disease"
...
"""
    ...
    try:
        ...
        # Fallback response for when API key is suspended
        mock_response = "I am currently in fallback mode because the API key is suspended. Irregular periods can be caused by stress, hormonal changes, or lifestyle factors. If they persist, it's best to consult a healthcare provider."
